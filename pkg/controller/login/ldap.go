...
import (
	...
	"github.com/zxl00/go-cheetah-admin/internal/model/response"
	modeSystem "github.com/zxl00/go-cheetah-admin/internal/model/system"
...
func GetLoginUserResource(id int, username string, ctx context.Context) (error, interface{}) {
	// 基础用户信息  LoginUser
	...
	sort.Slice(afterRemoveMenu, func(i, j int) bool {
		if afterRemoveMenu[i].ParentId == afterRemoveMenu[j].ParentId {
			return afterRemoveMenu[i].Sort < afterRemoveMenu[j].Sort
		}
		return afterRemoveMenu[i].ParentId < afterRemoveMenu[j].ParentId
	})
	loginUser.Menus = buildMenuTree(afterRemoveMenu, 0)
	if username == "admin" {
		if err, menuList := system.NewSysMenu(ctx).List(); err == nil {
			loginUser.Menus = menuList.([]modeSystem.Menu)
		}
	}
