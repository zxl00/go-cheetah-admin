...
import (
	...
	"github.com/zxl00/go-cheetah-admin/internal/model/system"
...
func GetChildren(menu *system.Menu) error {
	if err := global.GORM.Where("parent_id = ?", menu.ID).Order("sort asc").
...
func hasChildren(menu *system.Menu) bool {
	var count int64
	if err := global.GORM.Where("parent_id = ?", menu.ID).Model(&system.Menu{}).Count(&count).Error; err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			return true
		}
		return false
